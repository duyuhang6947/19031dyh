...
sqlite3 *db;  //仅服务器使用,库存在则打开，不存在则创建
int flags = 0;
...
int process_client_request(int clientfd, MSG *msg)
{
	...
	case USER_LOGIN:
	case ADMIN_LOGIN:
		process_user_or_admin_login_request(clientfd, msg);
		break;
	case USER_MODIFY:
		process_user_modify_request(clientfd, msg);
		break;
	case USER_QUERY:
		process_user_query_request(clientfd, msg);
		break;
	case ADMIN_MODIFY:
		process_admin_modify_request(clientfd, msg);
		break;
	case ADMIN_ADDUSER:
		process_admin_adduser_request(clientfd, msg);
		break;
	case ADMIN_DELUSER:
		process_admin_deluser_request(clientfd, msg);
		break;
	case ADMIN_QUERY:
		process_admin_query_request(clientfd, msg);
		break;
	case ADMIN_HISTORY:
		process_admin_history_request(clientfd, msg);
		break;
	case QUIT:
		process_client_quit_request(clientfd, msg);
		break;
	default:
		break;
	}
...
int process_admin_query_request(int clientfd, MSG *msg)
{
	...
	if(sqlite3_get_table(db, sql, &resultp,&nrow,&ncolumn,&errmsg) != SQLITE_OK){
		...
		if(msg->flags != 1){  //全部查询的时候不知道何时结束，需要手动发送结束标志位
			//通知对方查询结束了
...
int main(int argc, const char *argv[])
{
	...
	//创建socket->填充->绑定->监听->等待连接->数据交互->关闭 
	...
	serveraddr.sin_family = AF_INET;   				 /* 网络类型*/
	serveraddr.sin_addr.s_addr = inet_addr(argv[1]);  /* 终端输入IP*/
	serveraddr.sin_port = htons(atoi(argv[2])); 		 /* 终端输入端口号，字节序转换*/
	
	//绑定网络套接字和网络结构体
	if(bind(serverfd, (struct sockaddr *)&serveraddr, ser_len) ==  -1){
		printf("bind failed.\n");
		exit(-1);
	}
	...
	printf("tempfds = readfds\n");
		...
		for(i = 0;i < nfds + 1; i++){
			if(FD_ISSET(i,&tempfds)){
				if(i == serverfd){
					//数据交互
					clientfd = accept(serverfd, (struct sockaddr *)&clientaddr, &cli_len);
					if(clientfd == -1){
						printf("clientfd failed.\n");
						exit(-1);
					}
					printf("IP: %s.\n",inet_ntoa(clientaddr.sin_addr));
					FD_SET(clientfd,&readfds);
					nfds = nfds > clientfd ? nfds : clientfd;
				}else{
